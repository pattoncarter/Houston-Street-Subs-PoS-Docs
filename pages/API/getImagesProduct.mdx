...
```
# Purpose
This API is used as a secure SQL query to retrieve all of the items from the Houston Street Subs MenuImages table which cintains all items displayed on the client side. These items are then individually mapped to the Product interface which is used to populate the item view in a more concise way. This api is functionally pretty much the same as getImages except it will connect these returned items to the Product interface.
