...
export async function GET(request: Request) {
  try{
    ...
    console.log('getImagesSand error:', error);
    return null
  }
}
```
# Purpose
This API is used as a secure SQL query to retrieve all of the Sand items from the Houston Street Subs MenuImages table which contains all Sand items displayed on the client Sand.

# Parameters
This API requires no parameters to call. It can be reached using /api/getImagesSand.
